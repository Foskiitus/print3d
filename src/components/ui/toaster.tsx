...
>(({ className, variant = 'default', ...props }, ref) => (
  <ToastPrimitive.Root
    ...
    className={cn(
      ...
      variant === 'destructive' ? 'border-red-800 bg-red-950 text-red-300' : 'border-border bg-card text-foreground',
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ToastPrimitive.Close ref={ref} className={cn('rounded-md p-1 opacity-70 hover:opacity-100', className)} {...props}>
    <X className="h-4 w-4" />
...
type ToasterToast = ToastProps & { id: string; title?: string; description?: string }
    ...
    case 'DISMISS_TOAST': {
      ...
      if (toastId) {
        if (!toastTimeouts.has(toastId)) {
          toastTimeouts.set(toastId, setTimeout(() => { dispatch({ type: 'REMOVE_TOAST', toastId }) }, 3000))
        }
      }
