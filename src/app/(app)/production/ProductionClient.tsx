...
import type { Product, ProductionLog } from '@/types'
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wide">Observações</th>
                </tr>
              </thead>
              <tbody>
                {logs.map(log => (
                  <tr key={log.id} className="border-b border-border last:border-0 hover:bg-accent/30 transition-colors">
                    <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{formatDate(log.date)}</td>
                    <td className="px-4 py-3 font-medium text-foreground">{log.product.name}</td>
                    <td className="px-4 py-3 text-right">
                      <span className="text-emerald-400 font-medium">+{log.quantity}</span>
                    </td>
